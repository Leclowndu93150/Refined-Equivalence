package com.leclowndu93150.refined_equivalence.block.entity.lifecycle;
...
/**
 * Lifecycle helper that mirrors the Replication RS2 addon to keep node initialization resilient.
 */
public final class Rs2NodeLifecycle {
    ...
    public Rs2NodeLifecycle(
        final EmcLinkBlockEntity owner,
    ...
    public void shutdown(final String reason, final boolean allowRestart) {
        ...
        if (level != null && !level.isClientSide() && rsNodeAttached) {
            try {
                ...
                logger.warn("Refined Equivalence: Failed to remove RS2 node during {}: {}", reason, e.getMessage());
            } finally {
                rsNodeAttached = false;
            }
        }
    ...
    private boolean canAttemptInitialization() {
        if (EmcLinkBlockEntity.isWorldUnloading()) {
            return false;
        }
        final Level level = owner.getLevel();
        if (level == null || level.isClientSide()) {
    ...
    private void handleInitializationCallback(final String reason) {
        ...
        final Level level = owner.getLevel();
        if (level == null || level.isClientSide() || isRemoved()
            || EmcLinkBlockEntity.isWorldUnloading()
            || state != Rs2LifecycleState.INITIALIZING) {
            return;
        }
        try {
            owner.onRsNodeInitialized();
    ...
    private void handleInitializationFailure(final String reason, final Exception exception) {
        logger.error("Refined Equivalence: RS2 node initialization failed ({}): {}", reason, exception.getMessage(), exception);
